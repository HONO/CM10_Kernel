...
static irqreturn_t felica_rfs_detect_interrupt(int irq, void *dev_id)
{
	#ifdef FEATURE_DEBUG_LOW
...
#ifdef FELICA_LED_SUPPORT
  FELICA_DEBUG_MSG("[FELICA_RFS] FELICA LED SUPPORT !!\n");
  rc= request_irq(gpio_to_irq(GPIO_FELICA_RFS), felica_rfs_detect_interrupt, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING , FELICA_RFS_NAME, NULL);
  if (rc)
  {
