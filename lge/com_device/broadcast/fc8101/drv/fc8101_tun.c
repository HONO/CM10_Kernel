...
#define FC8100_FREQ_XTAL_TUNER    26000		//2011-05-11 ella.hwang
//#define FC8100_FREQ_XTAL_TUNER    24000	//2011-05-11 ella.hwang
...
static int fc8100_write(HANDLE hDevice, u8 addr, u8 data)
{
	...
	tmp = data;
	res = tuner_i2c_rf_write(hDevice, addr, 1, &tmp, 1);
...
static int fc8100_read(HANDLE hDevice, u8 addr, u8 *data)
{
	...
	res = tuner_i2c_rf_read(hDevice, addr, 1, data, 1);
...
/*=======================================================
  ...
  SIDE EFFECTS
    None.
  =========================================================*/
int fc8101_tuner_init(HANDLE hDevice, u8 band_type )
{
	int res = BBM_NOK;
...
int fc8101_tuner_deinit(HANDLE hDevice)
{
	int res = BBM_NOK;
...
int fc8101_set_freq(HANDLE hDevice, u8 ch_num)
{
	int res = BBM_NOK;
	...
	PRINTF(NULL, "FC8101_SET_FREQ : RF_PLL_K2 : %x, RF_PLL_K1 : %x, RF_PLL_K0 : %x, RF_PLL_N : %x\n", data_0x16, (unsigned char)( k_val >> 8 ), (unsigned char)( k_val ), (unsigned char)( n_val ));
	if(res)
		return res;
...
int fc8101_get_rssi(HANDLE hDevice, s32 *rssi)
{
	int res = BBM_NOK;
	...
	RSSI = (LNA * 6) + (RFVGA) + ((GSF & 0x7) * 6) - (PGA / 4) + K;
