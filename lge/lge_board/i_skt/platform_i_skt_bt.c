/* arch/arm/mach-msm/lge/board-i_skt-bt.c
...
static int i_skt_bluetooth_toggle_radio(void *data, bool state)
{
	int ret;
	int (*power_control)(int enable);

	 printk(KERN_ERR "[LG_BTUI] %s : Called i_skt_bluetooth_toggle_radio",__func__);  	 	
...
static struct bluetooth_platform_data i_skt_bluetooth_data = {
	.bluetooth_power = i_skt_bluetooth_power,
	.bluetooth_toggle_radio = i_skt_bluetooth_toggle_radio,
};
...
#if 1 //i'm not sure why phone reset when bt toggle is added to  platform  [BT_TOGGLE]
	.dev = {
		.platform_data = &i_skt_bluetooth_data,	
#else
	.dev = {
		.platform_data = &i_skt_bluetooth_power,
#endif
	},		
};
...
static struct bluesleep_platform_data i_skt_bluesleep_data = {
...
static struct platform_device msm_bluesleep_device = {
	...
	.dev = {
		.platform_data = &i_skt_bluesleep_data,
	},	
};
